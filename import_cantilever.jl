...
function import_HR_GLS(filename1::String,filename2::String)
    elements = Dict{String,Vector{ApproxOperator.AbstractElement}}()
    gmsh.initialize()
...
function import_MF_Gauss(filename1::String)
    elements = Dict{String,Vector{ApproxOperator.AbstractElement}}()
    gmsh.initialize()
...
function import_HR_GLS_MMP(filename1::String,filename2::String)
    elements = Dict{String,Vector{ApproxOperator.AbstractElement}}()
    gmsh.initialize()
    ...
    typep = ReproducingKernel{:Linear2D,:□,:CubicSpline}
    # # type = ReproducingKernel{:Quadratic2D,:□,:CubicSpline}
    sp = RegularGrid(xᵖ,yᵖ,zᵖ,n = 3,γ = 5)
    elements["Ωᵖ"] = getElements(nodes_p, entities["Ω"], typep, integrationOrder_Ω, sp)
    elements["∂Ωᵖ"] = getElements(nodes_p, entities["Γ"], typep, integrationOrder_Γ, sp)
    elements["Ωᵍᵖ"] = getElements(nodes_p, entities["Ω"], typep,  integrationOrder_Ωᵍ, sp)
    elements["Γᵍᵖ"] = getElements(nodes_p, entities["Γᵍ"],typep,  integrationOrder_Γ, sp, normal = true)
    # typep = PiecewisePolynomial{:Constant}
    # typep = PiecewisePolynomial{:Linear2D}
    # typep = PiecewisePolynomial{:Quadratic2D}
    # elements["Ωᵖ"] = getPiecewiseElements(entities["Ω"], typep, integrationOrder_Ω)
    ...
    return elements, nodes,nodes_p,  sp, type, Ω 
end
