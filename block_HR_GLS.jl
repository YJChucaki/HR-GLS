...
using ApproxOperator.Elasticity: ∫vᵢbᵢdΩ, ∫vᵢtᵢdΓ,∫∫σᵢⱼσₖₗdΩ,∫σᵢⱼnⱼuᵢdΓ,∫∫∇σᵢⱼuᵢdΩ,∫σᵢⱼnⱼgᵢdΓ, Hₑ
...
elements, nodes, sp, type, Ω, nodes_c = import_HR_GLS("./msh/block_"*poly*"_"*string(ndiv)*".msh","./msh/block_"*poly*"_"*string(ndiv2)*".msh",ndiv)
end
...
# prescribe!(elements["Γᵍᵘ"],:g₂=>(x,y,z)->0.0)
# prescribe!(elements["Γᵍᵘ"],:g₃=>(x,y,z)->0.0)
# prescribe!(elements["Γᵍᵘ"],:n₁₁=>(x,y,z,n₁,n₂,n₃)->n₁₁(n₁,n₂,n₃))
# prescribe!(elements["Γᵍᵘ"],:n₂₂=>(x,y,z,n₁,n₂,n₃)->n₂₂(n₁,n₂,n₃))
# prescribe!(elements["Γᵍᵘ"],:n₃₃=>(x,y,z,n₁,n₂,n₃)->n₃₃(n₁,n₂,n₃))
...
𝑏ᵅ = ∫σᵢⱼnⱼgᵢdΓ=>(elements["Γˢ"],elements["Γᵘ"])
...
kˢˢ = zeros(6*nₛ*nₑₛ,6*nₛ*nₑₛ)
kˢᵘ = zeros(6*nₛ*nₑₛ,3*nᵤ)
kˢᵘᵘ = zeros(6*nₛ*nₑₛ,3*nᵤ)
kˢᵘⁿ  = zeros(6*nₛ*nₑₛ,3*nᵤ)
fˢ = zeros(6*nₛ*nₑₛ)
fˢⁿ = zeros(6*nₛ*nₑₛ)
fᵘ = zeros(3*nᵤ)
...
d = [kˢˢ (kˢᵘ+kˢᵘᵘ+kˢᵘⁿ);kˢᵘ'+kˢᵘᵘ'+kˢᵘⁿ' zeros(3*nᵤ,3*nᵤ)]\[(fˢ+fˢⁿ);-fᵘ]
d₁ = d[6*nₛ*nₑ+1:3:end]
d₂ = d[6*nₛ*nₑ+2:3:end]
d₃ = d[6*nₛ*nₑ+3:3:end]
dₛ₁₁ = d[1:6:6*nₛ*nₑₛ]
...
err1 = kˢˢ*dₛ + (kˢᵘ+kˢᵘᵘ)*dᵤ-fˢ
err11 = kˢˢ*dₛ + (kˢᵘᵘ)*dᵤ-fˢ
err111 = kˢˢ*dₛ + (kˢᵘ)*dᵤ-fˢ
# err2 = kˢᵘⁿ*dᵤ - fˢⁿ
# err3 = (kˢᵘ'+kˢᵘⁿ')*dₛ + fᵘ
err3 = (kˢᵘ'+kˢᵘᵘ'+kˢᵘⁿ')*dₛ + fᵘ
err31 = (kˢᵘᵘ'+kˢᵘⁿ')*dₛ + fᵘ
err311 = (kˢᵘ'+kˢᵘⁿ')*dₛ + fᵘ
# err4 = kˢˢ*dₛ + (kˢᵘ+kˢᵘⁿ)*dᵤ - (fˢ+fˢⁿ)
# err5 = kˢᵘ'*dₛ + fᵘ
