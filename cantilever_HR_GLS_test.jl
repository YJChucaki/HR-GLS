...
using ApproxOperator.Elasticity: ∫∫vᵢbᵢdxdy, ∫vᵢtᵢds, L₂, L₂𝑝, Hₑ_PlaneStress, Hₑ_PlaneStrain_Deviatoric,∫∫σᵢⱼσₖₗdxdy_PlaneStrian,∫∫σᵢⱼσₖₗdxdy,∫σᵢⱼnⱼuᵢds,∫∫∇σᵢⱼuᵢdxdy,∫σᵢⱼnⱼgᵢds,∫∫τ∇σᵢⱼ∇σᵢₖdxdy,∫∫τ∇σᵢⱼ∇σᵢₖdxdy_new,∫∫τ∇σᵢⱼ∇σᵢₖdxdy_Taylor,∫∫σᵢⱼσₖₗdxdy_Taylor, Hₑ_PlaneStrain_Dil, 𝐿₂_PlaneStrain_Pressure, Hₑ_PlaneStrain_Deviatoric,  ∫∫εᵈᵢⱼσᵈᵢⱼdxdy_PPP, ∫∫σᵢⱼσₖₗdxdy_PPP, ∫∫εᵢⱼσᵢⱼdxdy_PlaneStrian,∫∫Cᵢⱼₖₗεᵢⱼσᵢⱼdxdy_PPP, ∫∫σᵢⱼσₖₗdxdy_Taylor
...
# prescribe!(elements["Ωˢ"],:nₛ=>(x,y,z)->nₛ)

prescribe!(elements["Ωᵘ"],:τ=>(x,y,z)->β)
prescribe!(elements["Ωᵘ"],:ℎ=>(x,y,z)->ℎ) 
prescribe!(elements["Ωˢ"],:E=>(x,y,z)->E)
prescribe!(elements["Ωˢ"],:ν=>(x,y,z)->ν)
prescribe!(elements["Ωˢˢ"],:E=>(x,y,z)->E)
prescribe!(elements["Ωˢˢ"],:ν=>(x,y,z)->ν)
...
prescribe!(elements["Ωᵘ"],:b₁=>(x,y,z)->0.0)
prescribe!(elements["Ωᵘ"],:b₂=>(x,y,z)->0.0)
...
# 𝑎t = ∫∫σᵢⱼσₖₗdxdy_Taylor=>elements["Ωˢ"]
# d = ∫∫εᵢⱼσᵢⱼdxdy_PlaneStrian=>(elements["Ωˢ"],elements["Ωᵘ"])
𝑎ᵝ = ∫∫εᵈᵢⱼσᵈᵢⱼdxdy_PPP=>elements["Ωᵘ"]
...
@timeit to "assembly" begin
 ...
 # @timeit to "solve" pardiso(ps,d,k,f)
    d = [kˢˢ kˢᵘ;kˢᵘ' kᵘᵘ]\[fˢ;-fᵘ]
    # d = [2*kˢˢ kˢᵘ;kˢᵘ' zeros(2*nᵤ,2*nᵤ)]\[fˢ;-fᵘ]
    d₁ = d[3*nₛ*nₑ+1:2:end]
    d₂ = d[3*nₛ*nₑ+2:2:end]
    dₛ₁₁ = d[1:3:3*nₛ*nₑₛ]
    dₛ₂₂ = d[2:3:3*nₛ*nₑₛ]
