...
elements, nodes, nodes_p, sp, type = import_linear_mix("./msh/block_"*poly*"_"*string(ndiv)*".msh","./msh/block_"*poly*"_"*string(ndiv_p)*".msh",ndiv_p)
...
pressure = zeros(nₑ)
for (i,elm) in enumerate(elements["Ωᵘ"])
    𝓒 = elm.𝓒
    𝓖 = elm.𝓖
    ε₁₁ = 0.0
    ε₂₂ = 0.0
    ...
    for (i,ξ) in enumerate(𝓖)
     ...
     σ₁₁ = E/(1+ν)/(1-2*ν)*((1-ν)*ε₁₁ + ν*ε₂₂ + ν*ε₃₃)
     σ₂₂ = E/(1+ν)/(1-2*ν)*(ν*ε₁₁ + (1-ν)*ε₂₂ + ν*ε₃₃)
     σ₃₃ = E/(1+ν)/(1-2*ν)*(ν*ε₁₁ + ν*ε₂₂ + (1-ν)*ε₃₃)
